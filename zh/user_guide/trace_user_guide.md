...
注：执行 `rvtrace create` 命令时，必须配置以下两种组合参数之一：

- `-target target_name` `-base addr` （通过 RISC-V target 控制 Trace 组件, K3 只支持此方式。）

- `-dap dap_name` `-ap-num apn` `-baseaddr addr` （通过ARM DAP 控制 Trace 组件）
...
注： 执行 `cstrace create` 命令时必须配置以下两种组合参数之一：
...
OpenSBI 作为运行在 M 模式下的 RISC\-V Linux 固件，其数据在 M/S/U 模式下无法直接访问，因而不能直接进行解析。需通过选项 `--firmware <file>`  和  `--firmware-base <address>` 传入 OpenSBI ELF 文件辅助解析。
...
- Missing DSO
  ...
  linux menuconfig中启用`CONFIG_PROC_KCORE`，如果还是出现这个报错，检查perf编译选项，`libelf`要启用
